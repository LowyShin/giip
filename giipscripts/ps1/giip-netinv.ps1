<#
.SYNOPSIS
  giip v3에서 지원하는 기능으로서, Windows 서버에서 3D 네트워크 구성 시각화를 위한 정보를 수집하여 JSON으로 저장합니다.
  아래에 필수 값들을 정확하게 넣고 실행하면 giip KVS에 저장이 됩니다. factor는 netinv로 저장되므로 데이터를 확인후,
  giip v3에서 네트워크 구성도 보기 페이지로 이동하면 네트워크 구성이 보입니다.
...
# region 사용자 설정 (여기만 수정) ----------------------------------------
# kvsput(giipapi) 호출 시 수동으로 채워야 하는 값을 여기서 지정하세요.
# 파라미터로 넘기면 이 값들을 덮어씁니다.
$KVSConfig = @{
  Enabled      = $true             # ← 실행 시 기본적으로 업로드 활성화
  Endpoint     = "https://giipfaw.azurewebsites.net/api/giipApi"  # giipapi 엔드포인트
  FunctionCode = "ask to me for funccode" # Azure Function code
  UserToken    = "user token on login"              # usertoken / token 필드에 사용
  UserId       = "user login id"                                 # (옵션) user_id
  KType        = "lssn"                                           # KVS 타입
  KKey         = "71197"                                          # ← lssn 값 (필수)
  KFactor      = "netinv"                                         # 분류/팩터
  HostKey      = $env:COMPUTERNAME                                 # 호스트명 보관용(업로드에 사용하지 않음)
}
  ...
  if (Test-HasCommand 'Get-NetIPConfiguration') {
    foreach ($c in (Get-NetIPConfiguration -ErrorAction SilentlyContinue)) {
      $cfgs += [pscustomobject]@{
        ...
        gateways        = @($c.IPv4DefaultGateway, $c.IPv6DefaultGateway | Where-Object { $_ } | ForEach-Object { $_.NextHop })
        dns_servers     = @($c.DnsServer.ServerAddresses)
    ...
    foreach ($w in (Get-CimInstance Win32_NetworkAdapterConfiguration -Filter 'IPEnabled = TRUE' -ErrorAction SilentlyContinue)) {
      ...
      $cfgs += [pscustomobject]@{
        ...
        dhcp            = if ($w.DHCPEnabled) { 'Enabled' } else { 'Disabled' }
